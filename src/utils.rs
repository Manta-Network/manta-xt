...
/// Create client
pub async fn create_manta_client<T: Config>(url: &str) -> Result<Client<T>> {
    ClientBuilder::new()
        .set_url(url)
        .build()
        .await
        .context("Failed to connect to manta node.")
}
...
mod tests {
    ...
    use std::str::FromStr;
    use subxt::{extrinsic::ExtrinsicParams, DefaultConfig};

    #[test]
    fn create_signer_from_seed_should_work() {
        let seed = "//Alice";
        let signer = create_signer_from_string::<DefaultConfig, sr25519::Pair>(seed).unwrap();
        assert_eq!(
    ...
    async fn transfer_balances() {
        ...
        let api = create_manta_client::<DefaultConfig>(url)
            .await
            .unwrap()
            .to_runtime_api::<crate::dolphin_runtime::RuntimeApi<
            DefaultConfig,
            crate::MantaExtrinsicParams<DefaultConfig>,
        >>();
        ...
        let extrinsic = api
            ...
            .unwrap();
        let tx_hash = extrinsic.sign_and_submit_default(&signer).await.unwrap();
    ...
    async fn batch_calls() {
        ...
        let batched_extrinsic = api.tx().utility().batch(vec![call]).unwrap();
        let tx_hash = batched_extrinsic
    ...
    async fn get_proposals() {
        ...
        let api = create_manta_client::<DefaultConfig>(url)
            .await
            .unwrap()
            .to_runtime_api::<crate::calamari_runtime::RuntimeApi<
            DefaultConfig,
            crate::MantaExtrinsicParams<DefaultConfig>,
        >>();

        let proposals = api.storage().council().proposals(None).await;
        dbg!(proposals);
        ...
        let proposals = api
            .storage()
            .council()
            .proposal_of(&proposal_hash, None)
            .await;
        dbg!(proposals);
    }
}
